<template>
  ...
  <div v-else class="table">
    <div class="table-header">
      ...
      <div>
        <input v-model="filterTerms" placeholder="Name(Title) or Url" />
      ...
      <table>
        ...
        <tbody>
          <tr v-for="(ctt, i) in paginatedData" :key="i">
            ...
            <td>{{ ctt.fingerprintId }}</td>
            <td>
              <button @click="scanFiggerprint(ctt._id)"><i class="fa-solid fa-user-check"></i></button>
  ...
  <base-modal :modalActive="modalActive" :modalTitle="modalTitle" @close="toggleModal">
    <div class="data-display">
      ...
      <input type="number" v-model="fingerprintId" />
    </div>
...
export default {
  ...
  computed: {
    ...
    getFingerprint () {
      return this.$store.getters['track/backendFingerprint']
    },
    filterItems () {
      if (!this.filterTerm) {
      ...
      return this.getFingerprint.filter(data =>
  ...
  methods: {
    ...
    addClick () {
      this.toggleModal()
      this.modalTitle = 'Add Fingerprint'
      this.id = 0
      this.employeeId = ''
    ...
    async createClick () {
      ...
      await this.$store.dispatch('track/postFingerprint', newFingerprint)
    ...
    editClick (data) {
      this.toggleModal()
      this.modalTitle = 'Edit Fingerprint'
      this.id = data._id
    ...
    async updateClick () {
      ...
      await this.$store.dispatch('track/updateFingerprint', updateFingerprint)
      this.getFingerprintData()
      this.toggleModal()
    },
    async deleteClick (id) {
      await this.$store.dispatch('track/deleteFingerprint', id)
      this.getFingerprintData()
      this.toggleAlert()
    },
    async scanFiggerprint (id) {
      if (this.recentPost) {
        console.log('call get')

        const newTracking = {
          employeeFingerprintId: id,
          time: this.getCurrentDateTime()
        }
        await this.$store.dispatch('track/postTracking', newTracking)
      } else {
        console.log('call post get')
